...
class FeedDaoRedis(FeedDaoBase, RedisDaoBase):
    ...
    def _insert(self, meter_reading: MeterReading,
        ...
        fields = MeterReadingSchema().dump(meter_reading)
        pipeline.xadd(
            self.key_schema.global_feed_key(), fields, maxlen=self.GLOBAL_MAX_FEED_LENGTH
        )
        pipeline.xadd(
            self.key_schema.feed_key(site_id), fields, maxlen=self.SITE_MAX_FEED_LENGTH
        )
        # END Challenge #6
